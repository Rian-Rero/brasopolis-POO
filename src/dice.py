...
class Dice:
    ...
    def roll(self) -> int:
        ...
        """
        self._result = random.randint(1, 6)
        return self._result
    ...
    def draw(self, screen: pygame.Surface, interact: Any, zoom: float, offset: Tuple[int, int]) -> None:
        ...
        :param interact: Objeto de interação.
        :param zoom: Fator de zoom.
        :param offset: Deslocamento (x, y).
        """
        ...
        text_rect = text.get_rect(center=self._button_rect.center)
        screen.blit(text, text_rect)

        # Mostrar o resultado do dado, se existir
        if self._result is not None:
            result_text = font.render(f"{self._result}", True, BLACK)
    ...
    def handle_event(self, event: pygame.event.Event) -> Optional[int]:
        ...
        """
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Clique com o botão esquerdo do mouse
